(function($) {
   ...
   // Animate On Scroll
    ...
    const lightboxes = function() {
        $WIN.on('load', function() {
            $('.pano-lightbox-popup').on('click', function(e) {
                e.preventDefault();
                ...
                for (const [key, value] of Object.entries(JSON.parse(el.attr('data-viewport')))) {
                    config[key] = value;
                }

                pannellum.viewer('panorama', config);

                $('#panorama-modal').modal({clickClose: false});
                document.scrollingElement.scrollTop=bodyScrollTop;
